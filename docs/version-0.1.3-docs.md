...
## Title
PHASE 02 / MILESTONE 2.3: In-Program Assertion Runner and Dual Entry Hooks

## Quick Diagnostic Read

`v0.1.3` completes PHASE-02 selftest integration by introducing a reusable `SelfTest` runner and making it reachable from both required entry paths:
...
- Release type: pre-alpha patch milestone for PHASE-02 selftest integration
...
- Includes validation cases aligned with PHASE-01/PHASE-02 behavior:
...
- Added minimal CLI entry routing for this milestone:
  - `--selftest` and `--selftest-large` direct paths,
  - interactive menu with option `5` invoking selftest.
- Non-selftest menu actions are placeholder outputs and remain scheduled for PHASE-03.

3. `README.md`

- Bumped visible version/status to `v0.1.3`.
- Updated feature and roadmap sections to include MILESTONE-2.3 completion.
...
- `Main` currently implements minimal routing to satisfy MILESTONE-2.3; full menu action wiring is still pending PHASE-03.
...
1. Execute `build aes256-java/ PHASE 03 MILESTONE 3.1` to expand `Main` into full menu scaffolding while retaining selftest behavior.
2. Wire encrypt/decrypt menu actions and friendly error mapping in PHASE-03 MILESTONE 3.2.
